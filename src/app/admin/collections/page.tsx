...
export default function AdminCollectionsPage() {
  ...
  const [pendingDelete, setPendingDelete] = useState<CollectionInfo | null>(null);
  ...
  function openCreate() {
    setEditing(null);
    setForm({ ...EMPTY_FORM, sortOrder: String(data?.collections.length ?? 0) });
    setErrors({});
    setSubmitError(null);
    setUploadError(null);
    setUrlDraft("");
  ...
  function openEdit(collection: CollectionInfo) {
  ...
  function validate(): Record<string, string> {
    ...
    if (!form.title.trim()) e.title = "A display title is required.";
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (editing === null) {
        await apiFetch<{ collection: CollectionInfo }>("/api/admin/collections", {
          method: "POST",
      ...
      setSubmitError(
        err instanceof Error ? err.message : "Could not save the collection.",
      );
    } finally {
      setSaving(false);
    }
  ...
  async function confirmDelete() {
    ...
    setActionError(null);
    try {
      await apiFetch<{ ok: true }>(`/api/admin/collections/${pendingDelete.id}`, {
        method: "DELETE",
      });
      ...
      // "3 products still use this collection. Reassign them first."
      setActionError(
    ...
    <div>
      <header className="flex flex-wrap items-end justify-between gap-4">
        ...
        <Button size="sm" variant="gold" onClick={openCreate}>
          <Plus size={14} strokeWidth={1.75} /> Add Collection
        </Button>
      </header>
      ...
      {panelOpen && (
        <form ref={panelRef} onSubmit={handleSubmit} noValidate className="mt-8">
          ...
          <div className="mt-3 grid gap-4 border border-line bg-white p-5 sm:grid-cols-2">
            <div>
              <label htmlFor="clf-name" className={labelClass}>
                Name *
              </label>
              <input
                id="clf-name"
              ...
              <p className="mt-1 text-[11px] text-stone">
                The short name products and filters use.
              </p>
              {renaming && (
                <p className="mt-1 text-xs text-amber-700">
                  Renaming updates all products in this collection.
                </p>
              )}
              {fieldError("name")}
            </div>
            ...
            <div>
              <label htmlFor="clf-title" className={labelClass}>
                Display title *
              </label>
              <input
                id="clf-title"
            ...
            <div className="sm:col-span-2">
              <label htmlFor="clf-desc" className={labelClass}>
                Description *
              </label>
              <textarea
                id="clf-desc"
                ...
                placeholder="How the collection is introduced on the collections page."
            ...
            <div>
              <label htmlFor="clf-sort" className={labelClass}>
                Sort order
              </label>
              <input
                id="clf-sort"
            ...
            <div className="sm:col-span-2">
              ...
              <p className="mt-3 text-[11px] leading-relaxed text-stone">
                The hero image on the collections page. Uploads go straight to
      ...
      <div className="mt-3 overflow-x-auto border border-line bg-white">
        {loading && !data ? (
          ...
          <p className="px-4 py-10 text-center text-sm text-stone">
            No collections yet — add the first one to curate the storefront.
          </p>
        ) : (
            ...
            <tbody>
              {data.collections.map((c) => (
                ...
                >
                  ...
                  <td className="max-w-xs truncate px-4 py-2.5 text-stone" title={c.title}>
                    {c.title}
                  </td>
      ...
      <ConfirmDialog
        ...
        message={
          pendingDelete
            ? `“${pendingDelete.name}” will be removed from the storefront permanently. A collection still used by products cannot be deleted.`
            : ""
        }
        confirmLabel="Delete"
        busy={deleting}
